...
namespace SteamKit2
{
    ...
    /// <summary>
    /// Scratches for latest NET Core
    /// </summary>
    public static class StreamScratches
    {
        public static void WriteNullTermString(Stream f, string value, Encoding encode)
        {
            var val = encode.GetBytes(value);
            f.Write(val, 0, val.Length);
            f.Write(new byte[] { 0 }, 0, 1);
        }

        /*public static string ReadNullTermString(Stream f, Encoding encode)
        {
            var buffer = new byte[2048];
            var written = 0;

            using (var bw = new BinaryReader(f, encode, true))
            {
                bw.
            }
            while (f.CanRead)
            {
                buffer[written++] = f.ReadByte();
            }

            return encode.GetString(buffer, written);     
        }*/

        public static ulong ReadUInt64(Stream f) 
        {
        ...
        public static long ReadInt64(Stream f)
        {
            using (var br = new BinaryReader(f))
            {
                return br.ReadInt64();
        ...
        public static int ReadInt32(Stream f)
        {
            using (var br = new BinaryReader(f))
            {
                return br.ReadInt32();
        ...
        public static float ReadFloat(Stream f)
        {
            using (var br = new BinaryReader(f))
            {
        ...
        public static string ReadNullTermString(Stream f, Encoding encode)
        {
            ...
            int b;
            while ((b = f.ReadByte()) != 0x00)
                strBytes.Add((byte)b);
            return encode.GetString(strBytes.ToArray());
        }
    }
}
